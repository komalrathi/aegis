      ...
      | Ok arg_types ->
          ...
          let arg_types_env_result =
            List.map arg_types_env ~f:(fun (arg_type, arg_expr) ->
                ...
                (* needs to be structural equality - look for function in
                   Core *)
          ...
          (* get the return type of the function from the type environment *)
          let return_type = List.last_exn arg_types in
          Ok
            ( return_type
