...
let type_function_defn
    (Parsed_ast.FunctionDefn
       ( fn_name
       , args
       , (return_core_type, return_security_level_type)
       , expr_body ) ) type_env =
  let ( >>= ) = Result.( >>= ) in
  ...
  in
  let fn_type = TFunction (arg_core_types, return_core_type) in
  type_expr expr_body
    ( (fn_name, (fn_type, return_security_level_type))
    :: (arg_types_env @ type_env) )
    TSLow
  >>= fun (expr_body_type, typed_expr_body, _) ->
