...
let rec type_program_function fn_defns expr type_env typed_class_defns pc row
    =
  let ( >>= ) = Result.( >>= ) in
  match fn_defns with
  | fn :: fn_defns ->
      type_function_defn fn type_env typed_class_defns row
      >>= fun (fn_name, fn_type, typed_f_defn, updated_row) ->
      type_program_function fn_defns expr
        ((fn_name, fn_type) :: type_env)
        typed_class_defns pc updated_row
      >>= fun (typed_fn_defns, typed_expr) ->
      Ok (typed_f_defn :: typed_fn_defns, typed_expr)
  | [] ->
      type_expr expr type_env typed_class_defns pc row
      >>= fun (_, typed_expr, _, _) -> Ok ([], typed_expr)
