...
module MenhirBasics = struct
  ...
  type token = 
    ...
    | MINUS
    | INT of (
# 6 "src/lex_and_parse/parser.mly"
       (int)
# 18 "src/lex_and_parse/parser.ml"
  )
    | EOF
    | DIVIDE
...
# 1 "src/lex_and_parse/parser.mly"
  
    open Ast_types 
    open Parsed_ast

# 32 "src/lex_and_parse/parser.ml"
...
let _menhir_action_1 =
  fun _startpos_i_ i ->
    ...
    (
# 39 "src/lex_and_parse/parser.mly"
        (INT(_startpos, i))
# 73 "src/lex_and_parse/parser.ml"
     : (Parsed_ast.expr))

let _menhir_action_2 =
  fun _startpos_e1_ e1 e2 ->
    let op = 
# 30 "src/lex_and_parse/parser.mly"
       ( PLUS )
# 81 "src/lex_and_parse/parser.ml"
     in
    let _startpos = _startpos_e1_ in
    (
# 40 "src/lex_and_parse/parser.mly"
                            (BINOP(_startpos, op, e1, e2))
# 87 "src/lex_and_parse/parser.ml"
     : (Parsed_ast.expr))

let _menhir_action_3 =
  fun _startpos_e1_ e1 e2 ->
    let op = 
# 31 "src/lex_and_parse/parser.mly"
        ( MINUS )
# 95 "src/lex_and_parse/parser.ml"
     in
    let _startpos = _startpos_e1_ in
    (
# 40 "src/lex_and_parse/parser.mly"
                            (BINOP(_startpos, op, e1, e2))
# 101 "src/lex_and_parse/parser.ml"
     : (Parsed_ast.expr))

let _menhir_action_4 =
  fun _startpos_e1_ e1 e2 ->
    let op = 
# 32 "src/lex_and_parse/parser.mly"
           ( MULTIPLY )
# 109 "src/lex_and_parse/parser.ml"
     in
    let _startpos = _startpos_e1_ in
    (
# 40 "src/lex_and_parse/parser.mly"
                            (BINOP(_startpos, op, e1, e2))
# 115 "src/lex_and_parse/parser.ml"
     : (Parsed_ast.expr))

let _menhir_action_5 =
  fun _startpos_e1_ e1 e2 ->
    let op = 
# 33 "src/lex_and_parse/parser.mly"
         ( DIVIDE )
# 123 "src/lex_and_parse/parser.ml"
     in
    let _startpos = _startpos_e1_ in
    (
# 40 "src/lex_and_parse/parser.mly"
                            (BINOP(_startpos, op, e1, e2))
# 129 "src/lex_and_parse/parser.ml"
     : (Parsed_ast.expr))

let _menhir_action_6 =
  fun e ->
    (
# 36 "src/lex_and_parse/parser.mly"
              (e)
# 137 "src/lex_and_parse/parser.ml"
     : (Parsed_ast.expr))
